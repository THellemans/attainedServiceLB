function [k_indxs, l_indxs, r, greater, greater_or_eq, xi] = sorted_kl_indxs(SCV, f,type,Delta,RepMax,Bsize,threshold)
% type = 1 : SQ(d)-RTB
% type = 2 : SQ(d)-RE(T)
% type = 3 : SQ(d)-RTB
% type = 4 : LAS(d)
...
EX=1;
[alpha, S, ~, ~] = hyperexponential_init(EX, SCV, f);
r=ceil(RepMax/Delta);
...
elseif type == 7
    ...
    for i=1:length(k_indxs)
        xi(i)=E_cond(alpha, S, S_inv, b(k_indxs(i))) - b(k_indxs(i)) + (l_indxs(i)-1);
    end
elseif type == 8
    xi = l_indxs;
end
